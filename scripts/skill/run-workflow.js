...
function resolveManifest(manifestArg) {
  ...
  if (require('node:fs').existsSync(fromCwd)) return fromCwd;

  const fromPlugin = path.resolve(PLUGIN_ROOT, manifestArg);
  if (require('node:fs').existsSync(fromPlugin)) return fromPlugin;
...
function main() {
  ...
  if (!resolvedManifest || !require('node:fs').existsSync(resolvedManifest)) {
  ...
  let manifest;
  try {
    const raw = require('node:fs').readFileSync(resolvedManifest, 'utf8');
  ...
  if (!require('node:fs').existsSync(preparePath)) {
