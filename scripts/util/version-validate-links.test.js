...
const CLI = path.join(__dirname, 'version-validate-links.js');
...
test('module exports runValidateLinksCli (re-export from the shared lib)', () => {
  const mod = require('./version-validate-links');
...
test('CLI success path: no URLs in stdin body -> exit 0, OK message on stdout', () => {
  const res = run([], 'a plain changelog with no urls in it');
...
test('CLI success path: --file flag reads body from disk', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'version-validate-links-test-'));
