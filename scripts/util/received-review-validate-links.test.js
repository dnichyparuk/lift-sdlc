...
const CLI = path.join(__dirname, 'received-review-validate-links.js');
...
test('module exports runValidateLinksCli (re-export from the shared lib)', () => {
  const mod = require('./received-review-validate-links');
...
test('CLI success path: no URLs in stdin body -> exit 0, OK message on stdout', () => {
  const res = run([], 'a plain review response with no urls in it');
...
test('CLI success path: --file flag reads body from disk', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'received-review-validate-links-test-'));
