...
**Nothing described here existed as code at the time this ADR was written.** `learn-prepare.js`, `learn-apply.js`, and `learn-reject.js` were all net-new at that point, and `scripts/lib/config.js`'s `PROJECT_SECTIONS` contained only `version, jira, commit, pr, plan, execute` — no `rejected_guardrails` or `learn` section. `scripts/ci/validate-guardrails.js` already existed but only validated schema (id format, uniqueness, description, severity); it had no notion of comparing two versions of a config against each other — that logic did not exist anywhere in this repo at ADR time and was built new for D7. This ADR is retained as the historical spec that the implementation now shipping alongside it was built from; consult the actual scripts under `scripts/skill/` and `scripts/lib/config.js` for current repo state rather than this framing paragraph.
